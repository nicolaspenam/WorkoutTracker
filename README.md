...
- **kg / lb** toggle (history stays stored in pounds so old sessions keep their numbers)
...
- Optional **RPE** per set and a one-line **note** per exercise
...
4. Enter weight and reps, or tap **✓** to fill last time’s numbers for that set. Empty sets with history show **Try …** for a small bump. RPE and a note are optional and do not start rest.
